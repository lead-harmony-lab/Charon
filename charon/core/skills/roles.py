...
System Version: v0.6.5 | File Revision: 9.1.2
...
class RoleResolverMixin:
    ...
    def get_default_agent_id(self, conn: Optional[sqlite3.Connection] = None) -> str:
        """
        ...
        """
        agent_id = self._role_repo.get_default_agent_id(conn=conn)
        if agent_id:
            return agent_id
    ...
    def validate_core_roles(self, conn: Optional[sqlite3.Connection] = None) -> bool:
        """
        ...
        """
        rows = self._role_repo.get_core_roles_status(conn=conn)
        if not rows:
            logger.error(
    ...
    def resolve_agent_id_for_role(
        self, role_input: str, conn: Optional[sqlite3.Connection] = None
    ) -> str:
        """
        ...
        resolved_id = self._role_repo.get_agent_id_for_role(sanitized_role, conn=conn)
        if resolved_id:
            return resolved_id
    ...
    def resolve_role(
        ...
        return self.resolve_agent_id_for_role(role_input, conn=conn)

    def get_agent_entrypoint(
        ...
        """
        canonical_id = self.resolve_agent_id_for_role(agent_id, conn=conn)

        entrypoint_data = self._role_repo.get_agent_entrypoint_data(canonical_id, conn=conn)
    ...
    def get_display_name_for_agent(
        self, agent_id: str, conn: Optional[sqlite3.Connection] = None
    ) -> str:
        """Retrieves human-readable display_name for an agent directly from DB."""
        canonical_id = self.resolve_agent_id_for_role(agent_id, conn=conn)

        display_name = self._role_repo.get_agent_display_name(canonical_id, conn=conn)
        if display_name:
            return display_name
