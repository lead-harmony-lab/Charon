...
router = APIRouter(tags=["Concierge HUD Stream"])
...
async def websocket_endpoint(
    ...
    logger.info(f"[WS STREAM CONNECT] Connected client_id='{client_id}' | Socket ID={id(websocket)} | Manager active_connections count={len(manager.active_connections)}")

    try:
        await manager.send_event(websocket, WSEvent.model_construct(
            event_type="status_change", agent_name="System", client_id=client_id,
            data={"status": "connected", "client_id": client_id, "message": "Connected to Charon Autonomic Stream Gateway"}
        ))
        while True:
