...
def build_concierge_stream_event(
    event_type: str, text: str, suggested_action: Optional[str] = None,
    emotion: str = "attentive", viseme_sequence: Optional[list] = None,
    target_app: Optional[str] = None, urgency: str = "medium", pointer_target: Optional[Dict[str, float]] = None
) -> Dict[str, Any]:
    event = ConciergeStreamEvent(
        type=event_type,
        payload={
            "text": text, "suggested_action": suggested_action,
...
async def handle_incoming_ws_frame(websocket: WebSocket, raw_data: str, client_id: Optional[str], manager: Any):
    """Processes incoming frames with full telemetry routing diagnostics."""
        ...
        if action in ("ping", "heartbeat"):
            ...
            if daemon and hasattr(daemon, "journal"):
                await daemon.journal.record_and_enqueue({
                    "task_id": task_id, "client_id": effective_client_id,
                    "prompt": msg.get("prompt", ""), "agent_override": msg.get("agent_override"), "context": msg.get("context", {})
                })
                await manager.send_event(websocket, WSEvent.model_construct(
                    event_type="status_change", task_id=task_id, agent_name="System", client_id=effective_client_id,
            ...
            if daemon and hasattr(daemon, "gatekeeper") and daemon.gatekeeper and approval_id:
                if hasattr(daemon.gatekeeper, "resolve_intercept"): daemon.gatekeeper.resolve_intercept(approval_id, decision_str)
                elif hasattr(daemon.gatekeeper, "submit_decision"): daemon.gatekeeper.submit_decision(approval_id, decision_str)
            ...
            if concierge and user_text:
                ...
                await websocket.send_json(build_concierge_stream_event("interact_response", response_text, emotion="speaking", target_app=msg.get("target_app", "Desktop")))
            ...
            if concierge and hasattr(concierge, "sensor"):
                if context_type == "window":
                    concierge.sensor.log_window_context(payload.get("app_name", "Unknown"), payload.get("window_title", ""), payload.get("active_file_path"), payload.get("pid"))
                elif context_type == "ide":
                    concierge.sensor.log_ide_context(payload.get("editor", "VSCode"), payload.get("file_path", ""), payload.get("language", "python"), payload.get("selection_or_diff"), payload.get("diagnostics"))

        elif action == "cursor_motion":
            if concierge and hasattr(concierge, "sensor") and hasattr(concierge.sensor, "log_cursor_motion"):
                concierge.sensor.log_cursor_motion(x=msg.get("x", 0.0), y=msg.get("y", 0.0), active_element=msg.get("active_element"))
