...
async def handle_incoming_ws_frame(websocket: WebSocket, raw_data: str, client_id: Optional[str], manager: Any):
    ...
    try:
        ...
        if action == "client_ready":
            logger.info(f"[WS STREAM HANDSHAKE] Client '{client_id}' reports ready.")

            # Trigger the Hospitality sequence through the Concierge facade
            if concierge and hasattr(concierge, "trigger_hospitality"):
                logger.info(f"Initiating Concierge.Hospitality sequence for {client_id}")
                asyncio.create_task(concierge.trigger_hospitality(client_id=client_id))
            else:
                logger.warning("Concierge system missing or lacks 'trigger_hospitality' method. Skipping sequence.")
        ...
        # --------------------------------------------------------------------
            ...
            # Route through the concierge facade for memory updates
            if action_event == "dismiss" and concierge and hasattr(concierge, "memory_collection"):
                try:
                    concierge.memory_collection.update(
        ...
        # --------------------------------------------------------------------
            ...
            # Extract target from the payload instead of hardcoding
            target_client = msg.get("target_client") or payload.get("target_client")

            if not target_client:
                logger.warning(f"Dropped desktop_ipc frame from '{client_id}': No 'target_client' specified.")
                return
